...
Ribbon.prototype.initialize = function () {
      var shaderDefinition = {
        ...
        fshader: [
            ...
            "void main(void)",
            "{",
            "    float gray = 0.7; // Set the desired brightness value here (0.0 to 1.0)",
            "    float dotSize = 0.03; // Set the size of the white dots",
            "    float dotFrequency = 0.1; // Set the frequency of white dots",
            "    vec3 baseColor = rainbow(vAge / trail_lifetime);",
            "    float alpha = (1.0 - (vAge / trail_lifetime)) * 0.5;",
            "",
            "    if (mod(gl_FragCoord.x, dotFrequency) < dotSize || mod(gl_FragCoord.y, dotFrequency) < dotSize) {",
            "        gl_FragColor = vec4(1.0, 1.0, 1.0, alpha); // White dot",
            "    } else {",
            "        gl_FragColor = vec4(baseColor * gray, alpha); // Grayscale color",
            "    }",
            "}"
        ].join("\n")
    };
